  ...
  <div class="quotes">
    ...
    <div class="quotes__container">
      <template v-for="quote in quotes">
        <div class="quotes__quote-block" v-show="getVisibility(quote.text)" :key="quote.id">
  ...
  directives: {
    replace: {
      inserted(element, bindings) {
        const  textField = element.textContent;
        const  searchText = bindings.value;

        if (searchText) {
          const reg = new RegExp(searchText, 'flag');
          element.innerHTML = textField.replace(reg, (match) => `<span>${match}</span>`)
        }
      },
    }
  },
