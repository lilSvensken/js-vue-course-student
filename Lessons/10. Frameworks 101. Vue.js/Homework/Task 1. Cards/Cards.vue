    ...
    <div class="card" v-for="card in cards" :key="card.id">
      ...
      <div class="card-body">
        ...
        <div class="card\_\_buttons">
          <button class="btn btn-primary button-plus" @click="addCounter(card.id)">+</button>
          <button class="btn btn-primary button-minus" @click="decreaseCounter(card.id)">-</button>
          <div>{{ card.number }}</div>
        </div>
        <div class="card\_\_alert alert alert-danger" v-if="!decreaseIsAvailable(card.id)">
...
export default {
  ...
  methods: {
    addCounter(id) {
      if (this.decreaseIsAvailable(id) === true) {
