...
export default {
    ...
    data() {
        return {
            radius: 0,
            earthMass: 6 * Math.pow(10, 24),
            gravityConst: 6.67 * Math.pow(10, -11),
        }
    },
    computed: {
        speed() {
            return Math.round(Math.sqrt(this.gravityConst * this.earthMass / (this.radius * 1000)));
        }
    }
    // Конец
};
...
.satellite__radius input {
    width: 200px;
    margin: 0 15px;
}

.satellite__radius > span {
    font-size: 20px;
}
